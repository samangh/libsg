...
/* uv/win.h redefines NSIG, which changes the  size of asio's signal_state. So it's best to have
 * libuv and asio in different translation units */
#include <uv.h>
...
std::vector<sg::net::interface_details> interfaces()
{
    ...
    try {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            std::copy(std::begin(uv_interface[i].phys_addr), std::end(uv_interface[i].phys_addr),
                      std::begin(int_det.physical_address));
