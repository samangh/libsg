...
/* Tabular CRC32-C with polynomial 0x82f63b78U
 * Also from https://github.com/komrad36/CRC */
namespace {
...
uint32_t tabular_1_byte(const void* M, uint32_t bytes, uint32_t prev)
{
...
uint32_t tabular_2_bytes(const void* M, uint32_t bytes, uint32_t prev)
{
    ...
    for (uint32_t i = 0; i < bytes >> 1; ++i)
    {
        R ^= M16[i];
        R = (R >> 16) ^
            g_tbl[0 * 256 + uint8_t(R >> 8)] ^
            g_tbl[1 * 256 + uint8_t(R >> 0)];
    }
    return R;
}

uint32_t tabular_4_bytes(const void* M, uint32_t bytes, uint32_t prev)
{
...
uint32_t tabular_8_bytes(const void* M, uint32_t bytes, uint32_t prev)
{
    const uint32_t* M32 = (const uint32_t*)M;
    uint32_t R = prev;
    while (bytes)
    {
        ...
        R = g_tbl[0 * 256 + uint8_t(R2 >> 24)] ^
            g_tbl[1 * 256 + uint8_t(R2 >> 16)] ^
            g_tbl[2 * 256 + uint8_t(R2 >> 8 )] ^
            g_tbl[3 * 256 + uint8_t(R2 >> 0 )] ^
            g_tbl[4 * 256 + uint8_t(R  >> 24)] ^
            g_tbl[5 * 256 + uint8_t(R  >> 16)] ^
            g_tbl[6 * 256 + uint8_t(R  >> 8 )] ^
            g_tbl[7 * 256 + uint8_t(R  >> 0 )];
        bytes -= 8;
    }
    return R;
}

uint32_t tabular_16_bytes(const void* M, uint32_t bytes, uint32_t prev)
{
    const uint32_t* M32 = (const uint32_t*)M;
    uint32_t R = prev;
    while (bytes)
...
uint32_t crc32c(const void *data, uint32_t length) {
     ...
     *
     * Here, 'div' is the largest factor of 2 that the input data size is divisble by */
    auto div = length & (~length + 1);
    switch (div) {
    case 16:
        return ~tabular_16_bytes(data, length, initial_remainder);
    case 8:
        return ~tabular_8_bytes(data, length, initial_remainder);
    case 4:
        return ~tabular_4_bytes(data, length, initial_remainder);
    case 2:
        return ~tabular_2_bytes(data, length, initial_remainder);
    default:
        return ~tabular_1_byte(data, length, initial_remainder);
    }
