...
void tcp_listener::write(client_id id, std::vector<uint8_t> vec)
{
    auto count =vec.size();
    auto a = sg::make_shared_opaque_buffer<uint8_t>(count);
    std::memcpy(a.get(), vec.data(), count * sizeof(uint8_t));
