...
static constexpr uint32_t g_lut_amd[] =
    ...
    #define CRC_ITER_INTEL(i) case i:			    \
    crcA = _mm_crc32_u64(crcA, *(uint64_t*)(pA - 8*(i)));   \
    crcB = _mm_crc32_u64(crcB, *(uint64_t*)(pB - 8*(i)));   \
    crcC = _mm_crc32_u64(crcC, *(uint64_t*)(pC - 8*(i)));

    #define X0_INTEL(n) CRC_ITER_INTEL(n);
    #define X1_INTEL(n) X0_INTEL(n+1) X0_INTEL(n)
    #define X2_INTEL(n) X1_INTEL(n+2) X1_INTEL(n)
    #define X3_INTEL(n) X2_INTEL(n+4) X2_INTEL(n)
    #define X4_INTEL(n) X3_INTEL(n+8) X3_INTEL(n)
    #define X5_INTEL(n) X4_INTEL(n+16) X4_INTEL(n)
    #define X6_INTEL(n) X5_INTEL(n+32) X5_INTEL(n)
    #define X7_INTEL(n) X6_INTEL(n+64) X6_INTEL(n)
    #define CRC_ITERS_256_TO_2_INTEL() do {X0_INTEL(256) X1_INTEL(254) X2_INTEL(250) X3_INTEL(242) X4_INTEL(226) X5_INTEL(194) X6_INTEL(130) X7_INTEL(2)} while(0)

    #define CRC_ITER_AMD(i) case i:				\
    crcA = _mm_crc32_u64(crcA, *(uint64_t*)(pA - 8*(i)));	\
    crcB = _mm_crc32_u64(crcB, *(uint64_t*)(pB - 8*(i)));
    ...
    #define CRC_ITERS_128_TO_2_AMD() do {X0_AMD(128) X1_AMD(126) X2_AMD(122) X3_AMD(114) X4_AMD(98) X5_AMD(66) X6_AMD(2)} while(0)
...
uint32_t crc32c(const void *data, uint32_t no_bytes, uint32_t previous_value, bool invert_previous_value, bool invert_final_value)
{
    if (invert_previous_value)
        previous_value = ~previous_value;

    uint32_t result;
    switch (sg::cpu::current_cpu_vendor()) {
    case sg::cpu::cpu_vendor::Amd:
        result = option_14_golden_amd(data, no_bytes, previous_value);
        break;
    case sg::cpu::cpu_vendor::Intel:
        result = option_13_golden_intel(data, no_bytes, previous_value);
        break;
    default:
