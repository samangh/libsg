...
uint32_t _internal::crc32c_hardware_intel(const void *M, uint32_t bytes, uint32_t prev) {
    ...
    while (bytes >= LEAF_SIZE_INTEL) {
        ...
        crcA =
            _mm_crc32_u64(crcC, _mm_cvtsi128_si64(_mm_xor_si128(vA, vB)) ^ *(uint64_t *)(pC - 8));
